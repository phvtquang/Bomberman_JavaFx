...
public class Bomb extends ActiveEntity {
     ...
     * @param xUnit toạ độ đơn vị
     * @param yUnit toạ độ đơn vị
     * @param img hình ảnh
     */
    public Bomb(int xUnit, int yUnit, Image img, int bomberID, int radius) {
    ...
    public void update() {
        if (timeToExplode > 0) { // Chưa nổ
            ...
            setImg(Sprite.movingSprite(Sprite.bomb,Sprite.bomb_1,Sprite.bomb_2,timeToExplode, animationBeetWeen).getFxImage());
        } else { // Nổ rồi
            ...
            // Animation bom nổ
            setImg(Sprite.movingSprite(Sprite.bomb_exploded,Sprite.bomb_exploded1,Sprite.bomb_exploded2,_timeAfter, animationBeetWeen).getFxImage());
        }
    }
